...
const canvasThemeSelect = document.getElementById('canvasTheme');
const progressRingSVG = document.getElementById('progressRing');
function setCanvasSize(size) {
  canvas.width = size;
  canvas.height = size;
  progressRingSVG.setAttribute('width', size+20);
  progressRingSVG.setAttribute('height', size+20);
  redraw();
}
...
function setCanvasTheme(theme) {
  if (theme === 'default') {
    ...
    canvas.style.background = 'linear-gradient(135deg,#fbc2eb 0%,#a1c4fd 100%)';
  } else if (theme === 'gradient2') {
    canvas.style.background = 'linear-gradient(135deg,#43cea2 0%,#185a9d 100%)';
  } else if (theme === 'pattern1') {
    // Dots pattern
    const patternCanvas = document.createElement('canvas');
    patternCanvas.width = patternCanvas.height = 32;
    const pctx = patternCanvas.getContext('2d');
    pctx.fillStyle = '#a1c4fd';
    pctx.fillRect(0,0,32,32);
    pctx.fillStyle = '#fbc2eb';
    pctx.beginPath();
    pctx.arc(8,8,4,0,2*Math.PI);
    pctx.arc(24,24,4,0,2*Math.PI);
    pctx.fill();
    canvas.style.background = `url(${patternCanvas.toDataURL()})`;
  }
}
canvasThemeSelect.onchange = e => {
  setCanvasTheme(e.target.value);
};
// Set initial size/theme
setCanvasSize(parseInt(canvasSizeSelect.value,10));
setCanvasTheme(canvasThemeSelect.value);

// --- Animated Progress Ring ---
function getProgressAngle(pts) {
  if (pts.length < 2) return 0;
  // Use center of best-fit or canvas center
  let fit = fitCircleKasa(pts);
  let cx = fit ? fit.cx : canvas.width/2;
  let cy = fit ? fit.cy : canvas.height/2;
  let angles = pts.map(p => Math.atan2(p.y-cy, p.x-cx));
  // Normalize angles to [0,2pi), sort, and find max gap
  ...
  let covered = 2*Math.PI - maxGap;
  return Math.max(0, Math.min(1, covered/(2*Math.PI)));
}
function updateProgressRing() {
  ...
  const cx = size/2 + 10;
  const cy = size/2 + 10;
  const progress = getProgressAngle(points);
  const circ = 2*Math.PI*r;
  let ring = progressRingSVG.querySelector('circle');
  if (!ring) {
    ring = document.createElementNS('http://www.w3.org/2000/svg','circle');
    ring.setAttribute('fill','none');
    ring.setAttribute('stroke','#4e54c8');
    ring.setAttribute('stroke-width','8');
    ring.setAttribute('stroke-linecap','round');
    ring.setAttribute('opacity','0.25');
    progressRingSVG.appendChild(ring);
  }
  ring.setAttribute('cx',cx);
  ring.setAttribute('cy',cy);
  ring.setAttribute('r',r);
  ring.setAttribute('stroke-dasharray',circ);
  ring.setAttribute('stroke-dashoffset',circ - circ*progress);
  ring.setAttribute('opacity', progress > 0.01 ? '0.7' : '0.25');
}
// Call updateProgressRing in redraw and on resize
